...
import adminFlashSaleService from "@/apis/services/adminFlashSaleService";
import type {
...
export default function ProductPicker() {
  ...
  const { id: flashSaleId } = useParams<{ id?: string }>();

  // Receive initial data from the calling page
  const stateInitialSelectedIds = Array.isArray(
    location.state?.initialSelectedIds,
  )
    ? (location.state.initialSelectedIds as string[])
    : undefined;
  const routeReturnTo = flashSaleId
  ...
  const returnTo: string =
    typeof location.state?.returnTo === "string"
      ...
      : routeReturnTo;
  const listReturnTo: string | undefined =
    typeof location.state?.listReturnTo === "string"
      ? location.state.listReturnTo
      : undefined;
  ...
  const flashSaleContextQuery = useQuery({
    queryKey: ["admin-product-picker-flash-sale-context", flashSaleId],
    queryFn: () =>
      adminFlashSaleService.getById(flashSaleId!).then((res) => res.data),
    enabled: Boolean(flashSaleId && !stateInitialSelectedIds),
    staleTime: 5 * 60_000,
  });
  const loadingFlashSaleContext = Boolean(
    flashSaleId && !stateInitialSelectedIds && flashSaleContextQuery.isPending,
  );
  const fetchedInitialSelectedIds = useMemo(
    () =>
      (flashSaleContextQuery.data?.items || [])
        .map((item) => item.variantId)
        .filter((variantId): variantId is string => Boolean(variantId)),
    [flashSaleContextQuery.data?.items],
  );
  const initialSelectedIds =
    stateInitialSelectedIds || fetchedInitialSelectedIds;
  ...
  useEffect(() => {
    if (initialSelectedIdSet.size === 0) return;
    setSelectedMap((current) => {
      const nextEntries = Object.entries(current).filter(
        ([variantId]) => !initialSelectedIdSet.has(variantId),
      );
  ...
  const productsQuery = useQuery({
    ...
    queryFn: ({ signal }) =>
      adminProductService
        ...
        .then((res) => res.data),
    enabled: !loadingFlashSaleContext,
  ...
  const loading =
    loadingFlashSaleContext || (productsQuery.isPending && !productsQuery.data);
  ...
  const handleToggleVariant = useCallback(
    (product: AdminProductPickerItem, variant: AdminProductVariantSummary) => {
      setSelectedMap((prev) => {
        const next = { ...prev };
        if (next[variant.id]) {
  ...
  const handleSelectAllVariants = useCallback(
    (
      ...
      const selectableVariants = variants.filter(
        (variant) => !initialSelectedIdSet.has(variant.id),
      );
      ...
      setSelectedMap((prev) => {
        ...
        const allNewSelected =
          ...
          selectableVariants.every((variant) => !!prev[variant.id]);
    ...
    [createSelectedVariant, initialSelectedIdSet],
  );

  const selectedVariantIds = useMemo(
    () => new Set(Object.keys(selectedMap)),
    [selectedMap],
  );
  const selectedProductIds = useMemo(
    () =>
      new Set(Object.values(selectedMap).map((variant) => variant.productId)),
    [selectedMap],
  );
  ...
  const handleConfirm = () => {
    navigate(returnTo, {
      state: {
        [PICKER_RESULT_KEY]: Object.values(selectedMap),
        ...(listReturnTo ? { returnTo: listReturnTo } : {}),
      },
    });
  };
  ...
  const handleCancel = () => {
    navigate(returnTo, {
      state: listReturnTo ? { returnTo: listReturnTo } : undefined,
    });
  };
  ...
  return (
    <div className="flex h-full min-h-[calc(100vh-80px)] flex-col gap-4">
      ...
      <div className="sticky top-0 z-20">
        <div className="rounded-[28px] border border-slate-200 bg-white/95 p-4 shadow-sm backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/95 sm:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            {selectedCount > 0 && (
              <div className="flex items-center gap-2 self-start rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 dark:border-blue-500/20 dark:bg-blue-500/10">
                <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                <span className="text-sm font-medium text-blue-700 dark:text-blue-300">
                  <Trans
                    i18nKey="adminCatalog:productPicker.selectedVariants"
      ...
      <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-slate-800 dark:bg-slate-800/50 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-md text-muted">
          {selectedCount > 0 ? (
            <Trans
              i18nKey="adminCatalog:productPicker.footerSelected"
              values={{ count: selectedCount }}
              components={{ strong: <strong className="text-blue-600" /> }}
            />
          ) : (
            ...
            <div className="divide-y divide-slate-200 dark:divide-slate-700">
              <div className="hidden md:grid grid-cols-[minmax(0,1fr)_170px_140px_170px] gap-0 bg-slate-100 dark:bg-slate-800/60 text-13 font-bold uppercase tracking-wide text-muted border-b border-slate-300 dark:border-slate-700">
                <div className="px-4 py-3.5">
                  {t("productPicker.table.product")}
                </div>
                <div className="px-4 py-3.5 text-right border-l border-slate-300 dark:border-slate-700">
                  {t("productPicker.table.sold")}
                </div>
                <div className="px-4 py-3.5 text-right border-l border-slate-300 dark:border-slate-700">
                  {t("productPicker.table.stock")}
                </div>
                <div className="px-4 py-3.5 text-right border-l border-slate-300 dark:border-slate-700">
                  {t("productPicker.table.actions")}
                </div>
              </div>
              {products.map((product) => (
                <ProductPickerProductRow
                  ...
                  hasNewSelection={selectedProductIds.has(product.id)}
                  isExpanded={!!expandedProducts[product.id]}
                  onToggleProduct={handleToggleProduct}
                  onToggleSelectAll={handleSelectAllVariants}
