    ...
    <div className="space-y-5 sm:space-y-8">
      ...
      {methods.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 sm:p-12 text-center border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-sm sm:text-base text-slate-500">Bấm thêm thẻ mới để thanh toán nhanh hơn nhé.</p>
        </div>
      ) : (
            ...
            {methods.map(method => (
              ...
              >
                ...
                <div className="space-y-4 relative z-10">
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <p className="text-sm sm:text-base font-medium tracking-wider">{method.name}</p>
                    </div>
                    <div>
                      <p className="text-xs opacity-70 uppercase tracking-wider mb-1">Hết hạn</p>
                      <p className="text-sm sm:text-base font-medium tracking-wider">{method.expiry}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 flex sm:hidden items-center gap-2 relative z-10">
                  {!method.isDefault && (
                    <button onClick={() => setAsDefault(method.id)}
                      className="flex-1 px-3 py-2 bg-white text-slate-900 rounded-lg text-sm font-semibold hover:bg-slate-100 transition-colors">
                      Đặt mặc định
                    </button>
                  )}
                  <button onClick={() => deleteMethod(method.id)}
                    className="px-3 py-2 bg-red-500 text-white rounded-lg text-sm font-semibold hover:bg-red-600 transition-colors flex items-center gap-1.5">
                    <FiTrash2 /> Xóa
                  </button>
                </div>
              </motion.div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input type="text" placeholder="0000 0000 0000 0000" className="w-full h-11 sm:h-12 pl-11 sm:pl-12 pr-4 rounded-xl bg-slate-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-purple-500 font-mono tracking-widest text-sm sm:text-base" maxLength={19} />
              <FiCreditCard className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xl" />
            </div>
          </div>
          <div><label className="block font-medium mb-2">Tên in trên thẻ</label><input type="text" placeholder="NGUYEN VAN A" className="w-full h-11 sm:h-12 px-4 rounded-xl bg-slate-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-purple-500 uppercase text-sm sm:text-base" /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block font-medium mb-2">Ngày hết hạn</label><input type="text" placeholder="MM/YY" className="w-full h-11 sm:h-12 px-4 rounded-xl bg-slate-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-purple-500 text-center text-sm sm:text-base" maxLength={5} /></div>
            <div><label className="block font-medium mb-2">CVV/CVC</label><input type="password" placeholder="•••" className="w-full h-11 sm:h-12 px-4 rounded-xl bg-slate-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-purple-500 text-center tracking-widest text-sm sm:text-base" maxLength={3} /></div>
          </div>
