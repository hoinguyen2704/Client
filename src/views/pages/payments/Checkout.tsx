...
import { Button, Checkbox, IconButton, Modal, ModalCancelButton, PrimaryButton, Radio, TrashButton, ExpandToggle } from '@/components';
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-4 bg-white dark:bg-slate-900 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-md font-bold text-slate-800 dark:text-slate-100 mt-1">
              {voucher.discountType === 'PERCENTAGE'
                ? `Giảm ${voucher.discountValue}%`
                : `Giảm ${formatPrice(voucher.discountValue)}`}
            </p>
            {voucher.minOrderValue && (
              <p className="text-sm text-slate-500 mt-1">Đơn tối thiểu {formatPrice(voucher.minOrderValue)}</p>
            )}
          </div>
            {(validatedProductCoupon?.id === voucher.id || validatedShippingCoupon?.id === voucher.id) ? (
              <PrimaryButton
                ...
                variant="outline"
                className="h-8 px-3 text-sm shadow-none shrink-0"
              >
                Bỏ áp dụng
              </PrimaryButton>
            ) : (
              <PrimaryButton
                ...
                className="h-8 px-3 text-sm shadow-none hover:translate-y-0 shrink-0"
              >
                Áp dụng
              </PrimaryButton>
            )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-6 border border-slate-100 dark:border-slate-800">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              ...
              <Button
                ...
                className="group w-full sm:w-auto justify-center text-md text-purple-600 dark:text-purple-400 hover:text-purple-700"
              >
                Thêm địa chỉ mới
              </Button>
            </div>
          ...
          {(validatedProductCoupon || validatedShippingCoupon) && (
            <div className="space-y-2">
              {validatedProductCoupon && (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-md p-3 rounded-xl bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800">
                   <div className="flex flex-col">
                      <span className="text-sm text-purple-600 font-bold">Voucher Sản Phẩm</span>
              ...
              {validatedShippingCoupon && (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-md p-3 rounded-xl bg-blue-50/50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
                  <div className="flex flex-col">
                      <span className="text-sm text-blue-600 font-bold">Voucher Freeship</span>
                      <span className="text-slate-600 dark:text-slate-300">
      ...
      >
        <div className="space-y-5">
          <div className="rounded-2xl border border-slate-100 dark:border-slate-800 p-4 bg-slate-50/70 dark:bg-slate-800/30 space-y-3">
            <h3 className="text-md font-bold uppercase tracking-wide text-slate-600 dark:text-slate-300">Nhập mã</h3>
