          ...
          {activeTab === 'specs' && (
            <motion.div key="specs" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="w-full">
              {specEntries.length > 0 ? (
                    ...
                    <tbody>
                      {specEntries.map(([key, value], idx) => (
                        <tr key={key} className={idx % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-slate-50 dark:bg-slate-800/50'}>
                          <td className="py-2.5 sm:py-4 px-3 sm:px-4 md:px-6 text-xs sm:text-sm font-medium text-slate-500 w-1/3 border-b border-slate-100 dark:border-slate-800">{key}</td>
                          <td className="py-2.5 sm:py-4 px-3 sm:px-4 md:px-6 text-xs sm:text-sm font-bold border-b border-slate-100 dark:border-slate-800">{value}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-slate-500 text-sm text-center py-8">Chưa có thông số kỹ thuật cho sản phẩm này.</p>
              )}
            </motion.div>
          )}
          {activeTab === 'description' && (
            <motion.div key="desc" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}
              className="prose dark:prose-invert max-w-none prose-sm sm:prose-base prose-img:w-full prose-img:rounded-2xl prose-img:mx-auto prose-a:text-purple-600 w-full overflow-hidden [&_.cps-block-content]:!max-h-none [&>div>div]:!max-h-none">
              {product.description ? (
                <div dangerouslySetInnerHTML={{ __html: product.description.replace(/style="[^"]*max-height[^"]*"/g, '') }} />
              ) : (
                <p className="text-slate-500 text-sm">Chưa có mô tả cho sản phẩm này.</p>
              )}
            </motion.div>
          )}
          {activeTab === 'reviews' && (
            <motion.div key="reviews" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="w-full">
              ...
              <div className="flex flex-col md:flex-row gap-4 sm:gap-8 items-center p-4 sm:p-6 md:p-8 bg-slate-50 dark:bg-slate-800/50 rounded-2xl mb-6 sm:mb-8 border border-slate-100 dark:border-slate-700">
                <div className="text-center md:w-1/4">
                  ...
                  <div className="text-slate-500 text-xs sm:text-sm">{reviews} đánh giá</div>
                </div>
                ...
                <div className="md:w-1/4 flex flex-col items-center justify-center w-full">
                  <p className="text-xs sm:text-sm text-slate-500 text-center">Chỉ khách hàng đã mua và nhận hàng thành công mới có thể đánh giá.</p>
                </div>
              </div>
                  ...
                  {groupedFeedbacks.map(group => {
                    ...
                    return (
                      <div key={mainFb.id} className="p-3 sm:p-5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                <h4 className="font-bold text-xs sm:text-sm">{mainFb.userName}</h4>
                        ...
                        {mainFb.content && <p className="mt-3 text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{mainFb.content}</p>}
                        {mainFb.adminReply && (
                          <div className="mt-3 p-2.5 sm:p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-800/30">
                            ...
                            <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300">{mainFb.adminReply}</p>
                          </div>
                        )}
                        ...
                        {group.length > 1 && (() => {
                          ...
                          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                            <div className="flex items-center justify-between mb-2">
                               <p className="text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                            ...
                            {group[1].content && <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{group[1].content}</p>}
                            {group[1].adminReply && (
                              <div className="mt-3 p-2.5 sm:p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-800/30">
                                <p className="text-xs font-bold text-blue-600 dark:text-blue-400 mb-1 flex items-center gap-1"><FiMessageSquare /> Phản hồi từ shop</p>
                                <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300">{group[1].adminReply}</p>
                              </div>
                            )}
                          </div>
                          );
